...
// My solution:
const ipToNum = ip => parseInt(ip.split('.').map(e => parseInt(e)).map(e => e.toString(2).padStart(8, 0)).join(''), 2)

const numToIp = num => num.toString(2).padStart(32, 0).match(/\d{8}/g).map(e => parseInt(e, 2)).join('.')