...
function sabb(x, val, happ) {
    let splitedX = x.toUpperCase().split('');
    let regEx = /S|A|B|T|I|C|L/;
    let points = 0

    for (let i = 0; i < splitedX.length; i++) {
        regEx.test(splitedX[i]) ? ++points : points
    }
    return points + val + happ > 22 ? 'Sabbatical! Boom!' : 'Back to your desk, boy.'
}